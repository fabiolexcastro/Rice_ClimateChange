...
# EXTRACCION DE PERFILES FUTURO - MAXENT
# H. Achicanoy & F. Castro
...
current <- list.files('./_current', full.names = T, pattern='.asc$')
current <- raster::stack(current)
future  <- list.dirs('./_raster/_2050s', full.names = T, recursive=F) 
future  <- future[setdiff(1:length(future), grep(pattern='maxent.cache', x=future))]
future  <- lapply(1:length(future), function(i){list.files(future[[i]], full.names = T, pattern='.asc$')})
future  <- lapply(1:length(future), function(i){raster::stack(future[[i]])})

# Directorio de trabajo
crossValDir <- './Profiles/future'
if(!file.exists(crossValDir)){dir.create(crossValDir)}
gcmList <- list.dirs('./_raster/_2050s', full.names = F, recursive=F)

# Crear particiones en los datos por GCM

fit.future <- lapply(1:length(future), function(i){
  
  gcmDir <- paste(crossValDir, '/', gcmList[[i]], sep='')
  if(!file.exists(gcmDir)){dir.create(gcmDir)}
  
  tryCatch(expr={
    fit <- dismo::maxent(x=future[[i]],
                         p=occ[,c("Long","Lat")],
...
# Definir nombres de variables, extracción de mínimos, máximos, Q1, Q2 y Q3
...
# Correr en paralelo variando por GCM

library(parallel)

runProfile <- function(gcm)
{
...
#     points(coords)
    ...
    listFitted <- apply(sapply(1:length(rList), function(x){
      ...
      z <- rList[[x]]
      names(z) <- names(future[[gcm]])
      predRaster <- predict(fit.future[[gcm]], z)
      ...
      y <- y$Idoneidad
      write.csv(mainResults, paste(crossValDir, '/', gcmList[[gcm]], '/fitted_values_maxent_',namPredic[j],'.csv', sep=''), row.names=F)
      return(y)}),1,median)
    ...
    profDir <- gcmDir <- paste(crossValDir, '/', gcmList[[gcm]], '/profilesPlots', sep='')
...
# source('H:/Trabajo_de_Grado/Maxent/Colombia/Profiles/mclapply2.R') # Hard disk
source('/mclapply2.R') # Hard disk

mclapply2(1:length(gcmList), fun=runProfile)
